...
import { useGetAlertsQuery, useGetProfileQuery, useUpdateWatchlistMutation } from "../../api";
import { useGetTopEarningStocksQuery } from "../../api/stock";

export default function TopGainers({ navigation, route }) {
  const { data: user } = useGetProfileQuery({});
  const [updateWatchlist] = useUpdateWatchlistMutation();
  const { data: topEarningStocks, isLoading } = useGetTopEarningStocksQuery({ category: 'gainer', email: user?.email, token: "" });
  ...
  return (
    <ScrollView contentContainerStyle={globalStyle.scrollContainer}>
      {isLoading ? <Loading /> : <>
        ...
        <View>
          <SymbolListAction data={topEarningStocks} callback={(symbol: string, value: boolean) => handleWatchlist(symbol, value)} />
        </View>
      </>}
    </ScrollView>
  );
