...
import { BookInfoHori } from "../../components/strapi/books/BookInfoHori";
import { LinearGradient } from "expo-linear-gradient";
import { TextInput } from "react-native";
import { StatusBar } from "expo-status-bar";
import { useGetBooksQuery, useGetCarouselsQuery, useGetCategoriesQuery, useGetLatestBooksQuery, useGetTopRatedBooksQuery, useSearchBooksQuery } from "../../api/strapi/book";
...
type BookRoute = RouteProp<BookStackParams, "Books">;

const Books = () => {
    const route = useRoute<BookRoute>();
    const navigation = useNavigation<NavigationProp<BookStackParams>>();
    const { data: topRatedBooks } = useGetTopRatedBooksQuery({});
    const { data: latestBooks } = useGetLatestBooksQuery({});
    const { data: carousels } = useGetCarouselsQuery({});
    const { data: categories } = useGetCategoriesQuery({});
    const [searchText, setSearchText] = useState("");
    const { data: searchList } = useSearchBooksQuery({ searchText: searchText });
                    ...
                    : <>
                        ...
                        <View style={styles.section}>
                            <View style={styles.sectionHeader}>
                                ...
                                <TouchableOpacity style={styles.viewBtn} onPress={() => navigation.navigate("BookList", { type: "TopRated" })}>
                            ...
                            }}>
                                {
                                    categories?.map((value, index: number) => {
                                        ...
                                        return <TouchableOpacity
                                            ...
                                            onPress={() => navigation.navigate("BookList", { type: "Category", category: value })}
                                            key={index}>
                                            <LinearGradient
                        ...
                        <View style={styles.section}>
                            <View style={styles.sectionHeader}>
                                ...
                                <TouchableOpacity style={styles.viewBtn} onPress={() => navigation.navigate("BookList", { type: "Latest" })}>
                                    <Text style={{ color: "#0F69FE", fontSize: 14 }}>View all</Text>
                                </TouchableOpacity>
                            </View>
                            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{}}>
