...
type CloseAlertRoute = RouteProp<RootStackParams, "CloseAlert">;

const CloseAlert = ({ navigation }) => {
    ...
    const alert = useMemo<Schema["Alert"]["type"]>(() => {
        return alertsData?.find((a: Schema["Alert"]["type"]) => a.identifier === route?.params?.symbol)
    }, [alertsData]);
        ...
        <View style={{
            ...
            <View style={{
                ...
                <View style={[styles.section, { gap: 15 }]}>
                            ...
                            !!alert && cancelAlert({
                                ...
                                if (!data.error) {
                                    Toast.show({
                                        ...
                                        position: "top",
                                        text1: route.params?.symbol + " is closed",
                                    })
                                    navigation.goBack();
                                }
                            });
                    ...
                    >
                        <Text style={{ color: "#fff", fontSize: 16, fontWeight: 'bold' }}>Cancel alert</Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                        onPress={() => {
                            navigation.goBack();
                    ...
                    >
                        <Text style={{ color: "#2EBD85", fontSize: 16, fontWeight: 'bold' }}>Do not cancel</Text>
                    </TouchableOpacity>
                </View>
            </View>
            {/* Disclosure modal */}
                        ...
                        <TouchableOpacity
                            onPress={() => {
                                ...
                                Toast.show({
                                    type: 'Capiwise_Success',
                                    position: "top",
                                    text1: "You have set an alert for " + route.params?.symbol,
                                })
                            }}
