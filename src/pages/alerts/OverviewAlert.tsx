...
import { useCancelAlertMutation, useCreateAlertMutation, useGetAlertsQuery, useUpdateAlertMutation } from "../../api/alert";
...
type CloseAlertRoute = RouteProp<RootStackParams, "OverviewAlert">;

const OverviewAlert = ({ navigation }) => {
    const route = useRoute<CloseAlertRoute>();
    ...
    const [cancelAlert] = useCancelAlertMutation();

    const { data: summary, isLoading, isFetching, isError } = alert.etf
        ? useGetEtfStockSummaryQuery({ symbol: alert.identifier, email: user?.email, token: "" })
        : useGetStockSummaryQuery({ symbol: alert.identifier, email: user?.email, token: "" });
        ...
        <View style={{
            ...
            <View style={{
                    ...
                    <Card style={{
                        ...
                        <Card.Content style={{
                            ...
                            <View style={{
                                ...
                                <Text style={styles.cardText}>{alert.etf ? summary?.overview?.details.symbol : summary?.profile?.symbol} ({alert.etf ? summary?.overview?.details?.name : summary?.profile.name})</Text>
                            </View>
                            <View style={{
                                flexDirection: 'row',
                                justifyContent: 'space-between',
                                ...
                                <Text style={{
                                    ...
                                    variant="bodyMedium">
                                    ${getRoundOffValue(alert.etf ? summary?.overview?.day1Range?.change : summary?.day1Range?.change)} ({getRoundOffValue(alert.etf ? summary?.overview?.day1Range?.percentChange : summary?.day1Range?.percentChange)})%
                                </Text>
                            </View>
                            <View style={{
                                flexDirection: 'row',
                                ...
                                <Text style={styles.cardTextLarge}>${getRoundOffValue(alert.etf ? summary?.overview.day1Range.close : summary?.day1Range?.close)}
                                    <Text
                                        style={{
                                            ...styles.cardText,
                                            fontWeight: 'bold',
                                ...
                                {/* ### */}
                                <Text style={styles.cardTextSmall}
                                    variant="bodyMedium">Data as of {formatDateAnalysis(alert.etf ? "" : summary?.date)}</Text>
                            </View>
                        </Card.Content>
                    </Card>
                    <View style={{
                        ...
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                            ...
                            {/* ### */}
                            <Text style={{ fontSize: 16, color: 'white', fontWeight: '700' }}>{formattedDate(new Date(alert?.createdAt))}</Text>
                        </View>
                    </View>
                </View>
                {/* Save button */}
                <View style={[styles.section, { gap: 15 }]}>
                    ...
                    >
                        <Text style={{ color: "#fff", fontSize: 16, fontWeight: 'bold' }}>Replace alert</Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                        onPress={() => {
                    ...
                    >
                        <Text style={{ color: "#E2433B", fontSize: 16, fontWeight: 'bold' }}>Cancel alert</Text>
                    </TouchableOpacity>
                </View>
            </View>
            {/* Disclosure modal */}
                        ...
                        <TouchableOpacity
                            onPress={() => {
                                ...
                                Toast.show({
                                    ...
                                    position: "top",
                                    text1: "You have set an alert for " + alert.identifier,
                                })
                            }}
...
export default memo(OverviewAlert);